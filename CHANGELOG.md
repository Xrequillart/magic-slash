...
- **Plan**: Move the rework and share buttons into the sticky bar
...
- **Plan**: Open a line's discussion in a bubble under the pointer instead of a side rail

### Changed

- **Plan**: Draw the spec header card's text with the design system's text component
- **Plan**: Fill the spec's inner cards instead of outlining them
...
- **Desktop**: Log why an avatar could not be read

### Fixed

- **Desktop**: Stop serving a replaced avatar from cache for an hour
- **Desktop**: Stop reporting success for an avatar never written
- **Desktop**: Keep a saved avatar from reverting to the cached copy
...
- **Desktop**: The debug menu can simulate a broken machine setup, so the setup screens can be seen without breaking the machine
- **Desktop**: The quick settings sheet opens the full app settings, instead of making you leave the sheet to get there

### Changed

- **Desktop**: The tab strip is drawn by the design system rather than by hand in the renderer
...
- **Landing**: An unescaped apostrophe in the design system's `SelectIcon` page failed the production build. The site deploys again
...
- **Landing**: An unescaped apostrophe in the design system page failed the production build. The site deploys again
...
- **Desktop**: The banner saying an agent is already on a ticket now stands apart from the body below it, with no outline and a larger mark
...
- **Desktop**: A board card shows the ticket's priority beside its id, and wears a green mark and border when an agent is already on it
- **Desktop**: A ticket page carries a green banner when an agent is already working on it, with a button that opens that agent
- **Desktop**: The start button is withheld on a ticket that is finished or already has an agent
...
- **Desktop**: The Team link sits between Tasks and Skills in the sidebar

### Fixed

- **Desktop**: The `read:jira-user` scope is requested, so connecting an Atlassian account no longer fails on the step that verifies it
...
- **Desktop**: The traffic-light gutter closes up in fullscreen

### Changed

- **Desktop**: The parent branch is shown at the same type scale as the worktree branch

## [0.84.0] - 2026-08-27

### Added

- **Desktop**: A repository now wears the colour picked in its settings everywhere it appears — on the agent sidebar's cards, in the settings list, in the settings rail and on its own settings page

### Changed

- **Desktop**: Repositories use the same icon as the webapp, tinted, in place of the bare colour dot
...
- **Desktop**: Selecting lines in a review opens the comment box straight away, inline under those lines and the full width of the view, and every comment on a file stays open as you read it
- **Desktop**: An issue can be handed to an agent to discuss rather than to build — explain it, refine it or rewrite it, without starting the work

## [0.82.1] - 2026-08-26

### Added

- **Desktop**: An issue's GitHub link can be copied from the tasks list and from the issue's own page
...
- **Desktop**: A tasks page listing the open GitHub issues of every tracked repository

### Changed

- **Desktop**: The sidebar agent rows no longer show a project color dot
- **Docs**: The release skill forbids `sed -i` for version bumps

### Fixed

- **Desktop**: Task reload responses are sequenced so a stale reload can't overwrite a fresher one
...
- **Desktop**: Pull request comments are listed in the PR card
- **Desktop**: Local review comments are cleared once sent to the agent
