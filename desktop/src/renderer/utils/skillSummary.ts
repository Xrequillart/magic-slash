 ...
 * time, and this module is unit-tested without a React tree or a language. The webapp
 * carries the twin — `webapp/lib/skillSummary.ts` — with the same keys and the same
 * branches; the two builds cannot import each other, so they are kept identical and
 * both are covered by the root test suite.
 *
