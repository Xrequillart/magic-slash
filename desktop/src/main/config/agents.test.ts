...
import { createDefaultMetadata, mergeMetadata, createDefaultSchedule } from './agents'
...
describe('createDefaultSchedule', () => {
  it('should return a disabled schedule with sensible defaults', () => {
...
describe('updateAgentSchedule', () => {
  let updateAgentSchedule: typeof import('./agents').updateAgentSchedule
  let readAgents: typeof import('./agents').readAgents
  ...
  beforeEach(async () => {
    ...
    vi.resetModules()
    const mod = await import('./agents')
    updateAgentSchedule = mod.updateAgentSchedule
    readAgents = mod.readAgents
  })
  ...
  it('should set the schedule on an existing agent', () => {
    writeAgentsFile([{
      id: 'a1',
      name: 'Agent',
      repositories: [],
    ...
    updateAgentSchedule('a1', schedule)
    const agents = readAgents()
    expect(agents[0].schedule).toBeDefined()
    expect(agents[0].schedule!.enabled).toBe(true)
    expect(agents[0].schedule!.command).toBe('npm test')
    expect(agents[0].schedule!.frequency).toBe('daily')
    expect(agents[0].schedule!.time).toBe('08:00')
  })

  it('should replace an existing schedule', () => {
    writeAgentsFile([{
      id: 'a1',
      name: 'Agent',
      repositories: [],
    ...
    updateAgentSchedule('a1', newSchedule)
    const agents = readAgents()
    expect(agents[0].schedule!.command).toBe('new command')
    expect(agents[0].schedule!.frequency).toBe('weekly')
    expect(agents[0].schedule!.enabled).toBe(false)
  })
  ...
  it('should not modify agents if ID is not found', () => {
    ...
    updateAgentSchedule('nonexistent', schedule)
    const agents = readAgents()
    expect(agents[0].schedule).toBeUndefined()
  })
})

describe('clearAgentSchedule', () => {
  let clearAgentSchedule: typeof import('./agents').clearAgentSchedule
  let readAgents: typeof import('./agents').readAgents
  ...
  beforeEach(async () => {
    ...
    vi.resetModules()
    const mod = await import('./agents')
    clearAgentSchedule = mod.clearAgentSchedule
    readAgents = mod.readAgents
  })
  ...
  it('should remove the schedule from an agent', () => {
    writeAgentsFile([{
      id: 'a1',
      name: 'Agent',
      repositories: [],
    ...
    clearAgentSchedule('a1')
    const agents = readAgents()
    expect(agents[0].schedule).toBeUndefined()
  })

  it('should be a no-op if the agent has no schedule', () => {
    writeAgentsFile([{
      id: 'a1',
      name: 'Agent',
      repositories: [],
  ...
  it('should not modify agents if ID is not found', () => {
    ...
    clearAgentSchedule('nonexistent')
    const agents = readAgents()
    expect(agents[0].schedule).toBeDefined()
    expect(agents[0].schedule!.command).toBe('npm test')
  })
