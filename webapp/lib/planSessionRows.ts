 ...
 * last changed. Both are optional in principle, and a row with neither sorts last
 * rather than throwing the comparator off.
 *
 * CREATION FIRST, which is the reverse of what this did. It was update-then-creation,
 * and the list printed the same value, so the dates read straight down the column. The
 * list now prints creation — a plan touched this morning is not a plan from this
 * morning — and a list ordered by one date while showing another puts its dates
 * visibly out of order, which reads as a bug. One value, sorted and shown.
 *
 * `created_at` is `not null default now()` in the table, so the second arm only ever
