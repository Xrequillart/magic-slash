...
const PROPS: PropRow[] = [
  ...
  { name: 'aside · asideOpen', type: 'ReactNode · boolean', description: 'A panel in the MIDDLE of the window, out only while asideOpen. The sheet answers what fits in a circle; the app puts everything else — its whole settings page, tabbed — in here, and it opens from inside the menu rather than replacing it, so the tiles stay under your hand while you read it. Centred and not beside the column: the panel sat immediately left of the sheet first, which tied a page-sized thing to the edge the menu happens to hug. It is not modal — no veil, no focus trap, and a click beside it closes the whole menu. A slot for children’s reason: what is on it is the app’s, where it sits and that a click inside it does not dismiss the menu are this component’s.' },
